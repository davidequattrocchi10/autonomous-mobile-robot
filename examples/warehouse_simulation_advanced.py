"""
Warehouse AGV Emergency Replanning Demo. A forklift enters the warehouse and stops
mid-aisle at the robot's planned route (row 12, col 9). The robot detects the blocked
path via LiDAR. DWA local avoidance fails 3 consecutive times — no safe trajectory
exists in the narrow corridor. The system triggers automatic A* replanning from the
robot's current position. A fresh path routes around the stopped forklift. Delivery
completed. Components: A* clearance-aware planning + DWA local control + LiDAR
sensing + automatic emergency replanning.
"""
...
WAREHOUSE_CONFIG = {
    ...
    'forklift_blocking_col': 8,     # col where forklift stops to block A* path
    ...
    # Output paths
    'gif_path': 'images/warehouse_simulation_advanced.gif',
    'png_path': 'images/warehouse_simulation_advanced_final.png',
    'gif_fps': 10,
    'gif_dpi': 100,
}
...
def main():
    ...
    fig.suptitle(
        "Warehouse AGV — Emergency Replanning Demo",
    ...
    def update(frame: int):
        ...
        nonlocal goal_reached, replan_count, forklift_stopped
        ...
        if forklift_active and frame % cfg['forklift_speed_steps'] == 0:
            ...
            if forklift_obs.position[1] >= cfg['forklift_blocking_col']:
                forklift_obs.obstacle_type = ObstacleType.STATIC  # freeze movement
                forklift_obs.velocity = (0, 0)  # clear stale velocity — predictive
                forklift_stopped = True          # check uses this to predict (row,col)
        ...
        if forklift_active:
            fobs = obs_manager.obstacles[forklift_id]
            if fobs.obstacle_type == ObstacleType.STATIC:
                fork_cells = set(
                    (r, c) for (r, c) in fobs.get_occupied_cells()
                    if 0 <= r < env.height and 0 <= c < env.width
                )
                path_blocked_by_forklift = any(
                    (r, c) in fork_cells for (r, c) in path[waypoint_index:]
                )
